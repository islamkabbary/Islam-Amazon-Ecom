...
<body>
                    ...
                    <div class="card-body contacts_body">
                        <ul class="contacts">
                            @foreach ($chats as $chat)
                                <li class="chats-for-user" data-id="{{ $chat[0]->sender_id }}"
                                    data-name="{{ $chat[0]->sender->name }}">
                    ...
                    <div class="card-body msg_card_body" id="chatBody">
                        <div class="d-flex justify-content-end mb-4">
                            <div class="msg_cotainer_send">
                                {{-- content message --}}
                                <span class="msg_time_send"> {{-- time message --}} </span>
                            </div>
                        ...
                        <form action="{{ route('sendMessages') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="input-group">
                                <div class="input-group-append">
                                    <input type="file" name="file" class="input-group-text input-file attach_btn">
                                </div>
                                <textarea id="messageBody" class="form-control type_msg"
    ...
    <script>
        ...
        $(document).on('click', '.chats-for-user', function(event) {
            var self = $(this);
            $('#reciverId').val(self.attr('data-id'));
            $.get("{{ URL::to('chats') }}/" + self.attr('data-id'), function(data) {
                $('.chats-for-user').removeClass('active');
                console.log(data);
                self.addClass('active');
                $('#chatName').text('Chat with ' + self.attr('data-name'))
                $('#chatCount').text(data.length + ' Messages')
                $('#chatBody').text(" ")
                data.forEach(element => {
                    if (element.sender_id == self.attr('data-id')) {
